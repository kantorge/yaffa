...
return new class () extends Migration {
    ...
    public function up(): void
    {
        ...
        DB::statement("
                        ...
                        WHEN 'standard' THEN
                            CASE tt.name
                                WHEN 'withdrawal' THEN
                                    (SELECT a.currency_id FROM accounts AS a
                                     LEFT JOIN account_entities AS ae ON ae.config_id = a.id AND ae.config_type = 'account'
                                     WHERE ae.id = tds.account_from_id)
                                WHEN 'deposit' THEN
                                    (SELECT a.currency_id FROM accounts AS a
                                     LEFT JOIN account_entities AS ae ON ae.config_id = a.id AND ae.config_type = 'account'
                                     WHERE ae.id = tds.account_to_id)
                                ELSE NULL
                            END
                        WHEN 'investment' THEN
                            (SELECT a.currency_id FROM accounts AS a
                             LEFT JOIN account_entities AS ae ON ae.config_id = a.id AND ae.config_type = 'account'
                             WHERE ae.id = tdi.account_id)
                    END AS currency_id
            ...
            ) AS calculations ON t.id = calculations.id
            SET
                t.currency_id = calculations.currency_id,
                t.cashflow_value = calculations.cashflow_value;
        ");
    }
