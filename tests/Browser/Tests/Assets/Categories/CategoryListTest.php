...
namespace Tests\Browser\Tests\Assets\Categories;
...
class CategoryListTest extends DuskTestCase
{
    ...
    public function test_delete_button_behaviour()
    {
        ...
        AccountEntity::factory()
            ->for($user)
            ->for(
                Payee::factory()->withUser($user)->create(['category_id' => $payeeDefaultCategory->id]),
                'config'
            )
            ->create();
        ...
        $payeeWithPreferredCategory = AccountEntity::factory()
            ->for($user)
            ->for(
                Payee::factory()->withUser($user)->create(['category_id' => null]),
                'config'
            )
            ->create();
