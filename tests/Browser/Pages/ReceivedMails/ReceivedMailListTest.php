...
test('user can load the received mail list and use filters', function () {
...
test('user can interact with mail action buttons', function () {
    // Load the main test user
    $user = User::firstWhere('email', $this::USER_EMAIL);
    ...
    $this->actingAs($user);
        ...
        $browser = visit(route('received-mail.index'))
        ->waitFor(SELECTOR_TABLE_RECEIVED_MAILS);
