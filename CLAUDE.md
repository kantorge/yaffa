...
- **All recurrence-pattern evaluation (budgets and transaction schedules) must go through `RecurrenceRuleService`, never a hand-built `Recurr\Rule`.** It is the only call site that applies `by_day`/`by_month` ordinal-weekday patterns (e.g. "first Wednesday of every month") via `setByDay()`/`setByMonth()`; `TransactionSchedule::isActive()`, `getNextInstance()`, `occursOn()`, and `Budget` all route through it correctly. `Transaction::scheduleInstances()` does not — it builds an inline `Recurr\Rule` and silently ignores `by_day`/`by_month`, falling back to plain FREQ+INTERVAL. Known bug, not yet fixed; if you touch forecast/budget-chart projections, check this first. See `.ai/docs/features/budget-schedule-redesign/architecture.md`.
