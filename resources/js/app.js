...
if (
    (current === 'account-entity.create' ||
        current === 'account-entity.edit') &&
...
$(function () {
    ...
    $('#jump_to_account').on('change', function () {
        ...
        window.location.href = route('account-entity.show', {
            account_entity: this.value,
        });
    });
    ...
    $('.cancel.confirm-needed').on('click', function (event) {
        ...
        const $form = $(this).closest('form');
        const isDirty =
            $form.length && $form.serialize() !== $form.data('initialState');
