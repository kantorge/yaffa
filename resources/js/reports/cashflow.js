...
function reloadData() {
    ...
    pollUntilReady(() => fetch(url).then((response) => response.json()), {
...
function onCashflowDataReady(data) {
    ...
    if (data.debug && data.debug.length > 0) {
        ...
        const flaggedCount = data.debug.filter(
            ...
            data.debug.map((row) => ({
                ...
                'Exchange rate': row.is_base_currency
                    ...
                    : row.exchange_rate != null
                      ? row.exchange_rate
                      : '⚠ no rate found, used 1:1 fallback',
                'Rate source month':
                    row.rate_source_month != null
                        ? row.rate_source_month
                        : row.is_base_currency
                          ? '(base currency)'
                          : '⚠ no rate found, fallback to 1:1',
        ...
        const flaggedRows = data.debug.filter(
            (r) => r.flags && r.flags.length > 0,
        );
...
function initializeRateNoteAlert() {
    ...
    fetch(window.route('api.v1.users.me.preferences.get', { key: dismissKey }))
        ...
        .catch((error) =>
            console.error('Failed to fetch dismissal preference:', error),
        );
    ...
    if (dismissButton) {
        dismissButton.addEventListener('click', () => {
            fetch(
                window.route('api.v1.users.me.preferences.set', {
                    key: dismissKey,
                }),
                {
                    method: 'PUT',
