        ...
        >
            ...
            <div class="d-flex flex-wrap gap-3">
                <div class="form-check form-check-inline mb-0">
                    ...
                    <label
                        class="form-check-label small"
                        for="filter-no-similarity"
                    >
                        {{ __('No similarity') }}
                        <span
                            v-if="noSimilarityCount"
                            class="badge bg-secondary ms-1"
                            >{{ noSimilarityCount }}</span
                        >
                    </label>
                </div>
                <div class="form-check form-check-inline mb-0">
                    ...
                    <label
                        class="form-check-label small"
                        for="filter-has-duplicate"
                    >
                        {{ __('Has duplicate match') }}
                        <span
                            v-if="hasDuplicateCount"
                            class="badge bg-danger ms-1"
                            >{{ hasDuplicateCount }}</span
                        >
                    </label>
                </div>
                <div class="form-check form-check-inline mb-0">
                    ...
                    <label
                        class="form-check-label small"
                        for="filter-has-schedule"
                    >
                        {{ __('Has scheduled match') }}
                        <span
                            v-if="hasScheduleCount"
                            class="badge bg-success ms-1"
                            >{{ hasScheduleCount }}</span
                        >
                    </label>
                </div>
                <div class="form-check form-check-inline mb-0">
                            ...
                            >
                                ...
                                <td>
                                    <template v-if="draft.matched_payee">
                                        ...
                                        <i
                                            ...
                                            :title="
                                                __(
                                                    'High-confidence similarity match (:pct%)',
                                                    {
                                                        pct: Math.round(
                                                            draft.matched_payee
                                                                .similarity *
                                ...
                                <td class="text-nowrap" @click.stop>
                                    <button
                                        ...
                                        :title="
                                            __(
                                                'Finalize this draft as a transaction',
                                            )
                                        "
                                        @click="
                                            $emit(
                            ...
                            <tr v-if="isRawVisible(draft.draft_index)">
                                <td colspan="9" class="bg-body-secondary p-3">
                                    <div class="row g-3">
                                        ...
                                        <div
                                            class="col-12 col-lg-7 d-flex flex-column gap-3"
                                        >
                                            <div
                                                v-if="
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            draftCount() {
                return this.drafts.filter((d) => d.status === 'pending_review')
                    .length;
            },
            ignoredCount() {
            ...
            finalizedCount() {
                return this.drafts.filter((d) => d.status === 'finalized')
                    .length;
            },
            noSimilarityCount() {
                return this.drafts.filter((d) => this.matchFlags(d).hasNone)
                    .length;
            },
            hasDuplicateCount() {
                return this.drafts.filter(
                    (d) => this.matchFlags(d).hasDuplicate,
                ).length;
            },
            hasScheduleCount() {
                return this.drafts.filter((d) => this.matchFlags(d).hasSchedule)
                    .length;
            },
            hasAiDocumentCount() {
                return this.drafts.filter(
                    (d) => this.matchFlags(d).hasAiDocument,
                ).length;
            },
        },
        watch: {
        ...
        methods: {
            ...
            matchFlags(draft) {
                ...
                const hasAiDocument =
                    (draft.related_ai_documents || []).length > 0;
            ...
            duplicateBadgeClass(draft) {
                const score = this.strongestConfidence(
                    this.visibleDuplicates(draft),
                );
