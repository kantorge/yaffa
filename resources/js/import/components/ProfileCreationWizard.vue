    ...
    <div>
        ...
        <div class="mb-4">
            ...
            <div class="d-flex align-items-start">
                <template v-for="n in 4" :key="n">
                    <div
                        class="d-flex flex-column align-items-center wizard-step-item"
                    >
                        <div
            ...
            <template v-if="sampleFile !== null && headers.length > 0">
                <div class="d-flex gap-2 mb-2 flex-wrap align-items-center">
                    ...
                    <span class="border rounded px-2 py-1 bg-body-secondary">
                        ...
                        <span
                            :class="
                                detectedHasHeader ? 'text-success' : 'text-body'
                            "
                        >
                            {{
                                detectedHasHeader
        ...
        <div v-if="currentStep === 2">
            ...
            <div class="row g-2 mb-3">
                <div class="col-md-5">
                    ...
                    />
                    <div
                        v-if="validationErrors.profileName"
                    ...
                    <div class="form-check mt-1">
                        ...
                        <label for="sign-asis" class="form-check-label">
                            ...
                            <span class="text-muted"
                                >—
                                {{
                                    __('use the parsed signed value directly')
                                }}</span
                            >
                        </label>
                    </div>
            ...
            <div class="mb-2 mt-4 fw-bold">
                {{
                    __(
                        'Preview with current settings (first :count data rows):',
                        {
                            count: previewRowCount,
                        },
                    )
        ...
        <div v-if="currentStep === 3">
            ...
            >
                <span
                    :class="
                        hasDateMapped ? 'text-success' : 'text-body-secondary'
                    "
                >
                    <i
                        :class="
                ...
                <span
                    :class="
                        hasAmountMapped ? 'text-success' : 'text-body-secondary'
                    "
                >
                    <i
                        :class="
                ...
                <span
                    :class="
                        hasPayeeMapped ? 'text-success' : 'text-body-secondary'
                    "
                >
                    <i
                        :class="
            ...
            >
                <div class="small fw-semibold mb-2">
                    ...
                    <span class="fw-normal text-muted ms-1">
                        ({{ __('column:') }}
                        {{ displayHeaders[dateMappedIndex] }})
                    </span>
                </div>
                <DateFormatSelector
                        ...
                        <tr>
                            ...
                            >
                                <select
                                    ...
                                    "
                                    @change="
                                        updateMapping(i, $event.target.value)
                                    "
                                >
                                    <option value="ignore">
                                        {{ __('— Ignore —') }}
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            dateMappedIndex() {
                return this.columnMappings.findIndex(
                    (c) => c.canonical === 'date',
                );
            },
            ...
            autoDetectedDateSamples() {
                for (let i = 0; i < this.headers.length; i++) {
                    ...
                    const looksLikeDates = samples.some((v) =>
                        DATE_PATTERNS.some(
                            (p) => v && p.regex.test(String(v).trim()),
                        ),
                    );
        ...
        methods: {
            ...
            async processFile(file) {
                ...
                this.delimiterChoice = DELIMITER_CANDIDATES.includes(delim)
                    ? delim
                    : '__custom__';
                if (!DELIMITER_CANDIDATES.includes(delim))
                    this.customDelimiter = delim;
            ...
            duplicateWarningForColumn(index) {
                ...
                if (canonical === 'comment' || canonical === 'reference')
                    return '';
                ...
                try {
                    ...
                    if (err?.response?.status === 422) {
                        ...
                        if (errors) {
                            ...
                            this.aiError =
                                errors[firstKey]?.[0] || __('Invalid request.');
                        } else {
                            this.aiError =
            ...
            applyAiSuggestion(data) {
                // Parser settings
                if (data.delimiter) {
                    this.delimiterChoice = DELIMITER_CANDIDATES.includes(
                        data.delimiter,
                    )
                        ? data.delimiter
                        : '__custom__';
                ...
                if (
                    data.mapping_json &&
                    typeof data.mapping_json === 'object'
                ) {
                    this.columnMappings = this.columnMappings.map((col) => ({
                        ...col,
                        canonical:
                            data.mapping_json[col.header] ?? col.canonical,
                    }));
                }
