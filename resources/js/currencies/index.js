  ...
  columns: [
      ...
      render: function (data, type, row) {
        ...
        if (type === 'display') {
          return (
            data +
            (row.base
              ? ' <i class="fa fa-star text-primary" title="' +
                __('Base currency') +
                '"></i>'
              : '')
          );
    ...
    },
    {
      ...
      render: function (data, _type, row) {
        return (
          genericDataTablesActionButton(data, 'edit', 'currencies.edit') +
          // Base currency cannot be deleted or set as default
          (!row.base
            ? '<a href="/currencyrates/' +
              data +
              '/' +
              window.YAFFA.userSettings.baseCurrency.id +
              '" class="btn btn-xs btn-info" title="' +
              __('Rates') +
              '"><i class="fa-solid fa-fw fa-chart-line"></i></a> ' +
              genericDataTablesActionButton(data, 'delete') +
              '<a href="' +
              window.route('currencies.setDefault', data) +
              '" class="btn btn-xs btn-primary data-set-default" title="' +
              __('Set as default') +
              '"><i class="fa-solid fa-fw fa-building-columns"></i></a>'
            : '')
        );
      },
      className: 'dt-nowrap',
...
$(dataTableSelector).on('click', '.data-delete:not(.busy)', function () {
  ...
  confirmDelete(__('Are you sure to want to delete this item?')).then(
    (result) => {
      ...
      axios
        .delete(window.route('api.v1.currencies.destroy', id))
        .then(function () {
          window.currencies = window.currencies.filter(
        ...
        .catch(function (error) {
          toastHelpers.showErrorToast(
            error.response?.data?.error ||
              __('Error while trying to delete currency'),
          );
        })
        .finally(function () {
...
$(dataTableSelector).on('click', 'a.data-set-default', function (event) {
  ...
  confirmAction(
    __('Are you sure you want to set this currency as the default one?'),
  ).then((result) => {
    if (result.isConfirmed) {
