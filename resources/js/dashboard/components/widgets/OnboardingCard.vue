    ...
    <div v-show="ready" id="onboardingCardContainer" class="card mb-3">
        <div class="card-header d-flex justify-content-between">
            ...
            <div>
                <button
                    class="btn-close"
                    :disabled="busy"
                    :title="__('widget.onboardingCard.hideTemporarilyTitle')"
                    type="button"
                    @click="hide"
                ></button>
            </div>
            ...
            >
                <div class="d-flex justify-content-between">
                    <span>
                        <i
                            v-if="!step.complete"
                            class="text-primary fa-regular fa-square me-2"
                        ></i>
                        <i
                            v-if="step.complete"
                            class="text-success fa-regular fa-square-check me-2"
                        ></i>
                        {{ step.title }}
                    </span>
                    <span v-show="!step.complete" class="text-end">
                        <a v-if="step.link && step.cta" :href="step.link">{{
                            step.cta
                        }}</a>
        ...
        <div class="card-footer d-flex justify-content-between">
            ...
            <button
                ...
                :disabled="busy"
                :title="__('widget.onboardingCard.dismissTitle')"
                type="button"
                @click="dismiss"
            >
                {{ __('widget.onboardingCard.dismissButton') }}
            </button>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            axios
                ...
                .then((response) => {
                    ...
                    const tourData = response.data.steps.find(
                        (step) => step.tour,
                    );
                        ...
                        vue.tourData = driver({
                            ...
                            onDestroyStarted: () => {
                                if (!vue.tourData.hasNextStep()) {
                                    axios
                                        .post(
                                            `/api/v1/onboarding/${vue.topic}/complete-tour`,
                                        )
                                        .then(() => {
                                            vue.onboardingSteps.forEach(
                                                (step) => {
                                                    if (step.tour) {
                                                        step.complete = true;
                                                    }
        ...
        methods: {
            hide() {
                document
                    .getElementById('onboardingCardContainer')
