        ...
        data() {
            ...
            if (this.action === 'create') {
                if (urlParams.get('account')) {
                    data.transactionData.config.account_id =
        ...
        methods: {
            ...
            loadCallbackUrl(transaction) {
                ...
                if (this.callback === 'returnToInvestment') {
                    location.href = this.route('investments.show', {
            ...
            onSuccess(transaction, options) {
                if (
                    ['create', 'clone', 'enter', 'finalize'].includes(
                        this.action,
                    )
                ) {
                    storeNotification(
                    ...
                    if (options.investmentPriceStoredResult) {
                        if (options.investmentPriceStoredResult === 'success') {
                            storeNotification(
                                'success',
                                __('Investment price stored'),
                                {
                                    dismissible: true,
                                },
                            );
                        } else if (
                            options.investmentPriceStoredResult === 'skipped'
                        ) {
                            storeNotification(
                                'warning',
                                __(
                            ...
                            options.investmentPriceStoredResult === 'error'
                        ) {
                            storeNotification(
                                'error',
