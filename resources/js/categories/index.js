  ...
  columns: [
    ...
    },
    {
      ...
      render: function (data, _type, row) {
        return (
          genericDataTablesActionButton(data, 'edit', 'categories.edit') +
          renderDeleteAssetButton(
            row,
    ...
    $(settings.table).on('click', 'td.activeIcon > i', function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          categories.filter((category) => category.id === data.id)[0].active =
            data.active;

          toastHelpers.showSuccessToast(__('Category active state changed'));
        },
