...
window.categories = window.categories.map(function (category) {
    ...
    if (category.transactions_min_date) {
        category.transactions_min_date = new Date(
            category.transactions_min_date,
        );
    }
    // Parse last date if it exists
    if (category.transactions_max_date) {
        category.transactions_max_date = new Date(
            category.transactions_max_date,
        );
    }
...
const deleteButtonConditions = [
    ...
    },
    {
        ...
        negate: false,
        errorMessage: __(
            'It is used as not preferred category by some payees.',
        ),
    },
    {
            ...
            render: function (data, _type, row) {
                return (
                    ...
                    renderDeleteAssetButton(
                        ...
                        __('This category cannot be deleted.'),
                    ) +
                    '<a href="' +
                    route('categories.merge.form', { categorySource: data }) +
                    '" class="btn btn-xs btn-primary" title="' +
                    __('Merge into an other category') +
    ...
    initComplete: function (settings) {
        ...
        $(settings.table).on('click', 'td.activeIcon > i', function () {
            ...
            $.ajax({
                type: 'PATCH',
                url: window.route(
                    'api.v1.categories.patch-active',
                    row.data().id,
                ),
                data: JSON.stringify({
                    _token: csrfToken,
...
initializeDeleteAssetButtonListener(
    ...
    __('Category deleted'),
    function (id, tr) {
        window.categories = window.categories.filter(
