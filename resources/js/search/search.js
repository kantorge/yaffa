...
document
  .querySelectorAll('#table-search-results-accounts td.transactionCount')
  .forEach(getTransactionCount);

// Loop the span placeholder for all the payee results, and get the number of associated transactions for each payee.
document
  .querySelectorAll('#table-search-results-payees td.transactionCount')
  .forEach(getTransactionCount);

// Loop the span placeholder for all the tag results, and get the number of associated transactions for each tag.
document
  .querySelectorAll('#list-search-results-tags span.transactionCount')
  .forEach(getTransactionCount);

// Loop the span placeholder for all the category results, and get the number of associated transactions for each category.
document
  .querySelectorAll('#list-search-results-categories span.transactionCount')
  .forEach(getTransactionCount);

// Loop the span placeholder for transactions, and display the view and quick view icons for each transaction.
document
  .querySelectorAll('#table-search-results-transactions td.transactionIcon')
  .forEach(function (element) {
    const id = element.dataset.id;
    element.innerHTML =
      dataTableHelpers.dataTablesActionButton(id, 'quickView') +
      dataTableHelpers.dataTablesActionButton(id, 'show');
    element.classList.remove('hidden');
  });
...
app.component('TransactionShowModal', QuickViewTransactionModal);
