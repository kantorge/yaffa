        ...
        <ul class="list-group list-group-flush collapse show" aria-expanded="true" id="cardActions">
            ...
            <li class="list-group-item d-flex justify-content-between align-items-center">
                ...
                <button
                    ...
                    :disabled="isLoadingMissing"
                >
                    <span v-if="isLoadingMissing" class="spinner-border spinner-border-sm"></span>
...
export default {
    ...
    computed: {
        retrieveLatestUrl() {
            return window.route('currency-rate.retrieveRate', { currency: this.fromCurrency.id });
        },
    },
    methods: {
        addNewRate() {
        ...
        async loadMissingRates() {
            ...
            try {
                ...
                await window.axios.get(
                    window.route('currency-rate.retrieveMissing', { currency: this.fromCurrency.id })
                );
                ...
                const errorEvent = new CustomEvent('toast', {
                    detail: {
                        ...
                        body: error.response?.data?.message || this.__('Failed to load missing rates'),
                ...
                // Close loading toast
                setTimeout(() => {
                    const toastElement = document.querySelector('.toast-loading-missing-rates');
                    if (toastElement) {
