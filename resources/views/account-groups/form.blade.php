...
<form
    ...
    action="{{ route('account-groups.update', $accountGroup->id) }}"
    autocomplete="off"
    method="POST"
>
@method('PATCH')
...
<form
    accept-charset="UTF-8"
    action="{{ route('account-groups.store') }}"
    autocomplete="off"
    method="POST"
>
@endisset
    ...
    <div class="card">
        ...
        <div class="card-footer">
            ...
            <a href="{{ route('account-groups.index') }}" class="btn btn-secondary cancel confirm-needed">{{ __('Cancel') }}</a>
        </div>
    </div>
</form>
@stop
