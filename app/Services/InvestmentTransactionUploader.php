...
class InvestmentTransactionUploader
{
    ...
    public function mapTransaction(array $rawData, array $mapping, int $rowIndex): array
    {
        ...
        // Ensure quantity is float or null
        if (isset($transaction['config']['quantity'])) {
            if ($transaction['config']['quantity'] === '' || $transaction['config']['quantity'] === null) {
                $transaction['config']['quantity'] = null;
            } else {
                $transaction['config']['quantity'] = (float) $transaction['config']['quantity'];
            }
        }
    ...
    {
        // Handle cash-only transactions (Deposit/Withdrawal) - create or use a "Cash" placeholder investment
        if (isset($transaction['_is_cash_only']) && $transaction['_is_cash_only']) {
            // Look for or create a "Cash" investment for this account's currency
            $currencyId = null;
            ...
            // Find or create "Cash" investment
            ...
            $transaction['config']['quantity'] = null; // Cash transactions don't have quantity
            $transaction['config']['price'] = null; // Cash transactions don't have price
        ...
        // Resolve account by name or use default account
        ...
        // Resolve transaction type by name with aliases (case-insensitive)
        if (!$transaction['transaction_type_id'] && isset($transaction['_transaction_type_name'])) {
            ...
            if ($transactionType) {
                ...
                // For cash-only transactions (deposit/withdrawal), mark them specially
                if (in_array($normalizedType, ['deposit', 'withdrawal'])) {
    ...
    public function getDefaultMapping(string $source): array
    {
        return match($source) {
            ...
            'Trading212' => [
              ...
              //  'Total' => ['target' => 'config.dividend', 'transform' => 'float'], // Used for dividends and cash movements
