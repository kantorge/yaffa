...
| Bearer token cannot use unscoped abilities on other `/api/v1/*` endpoints | `permissions.md` § Full Ability Enforcement — Implementation Plan | A token without a route's required ability is denied on all ~24 remaining controllers; a token with it is not blocked by the ability check | `ApiAbilityEnforcementTest.php` (90 cases: one deny + one allow per representative route) | integration | existing |
...
- `ApiAbilityEnforcementTest` (data-provider-driven, 45 representative routes × 2 = 90 cases: one deny-without-ability, one allow-with-ability, per controller)
