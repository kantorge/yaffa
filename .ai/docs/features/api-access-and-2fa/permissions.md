...
| Recovery codes | One-time-use login fallback | Returned in the HTTP response body exactly once (at `confirm` and at `regenerateRecoveryCodes`), never re-servable afterward. Stored hashed, not reversibly encrypted. |
