...
| Step | Boundary crossed | Authz check | Side effect |
|---|---|---|---|
| 1. `POST /login` `{ email, password }` — `guest` + `throttle:6,1` middleware on the route. | Browser → server | `LoginController::validateLogin()`: no `_2fa_login` session key yet → validates full credential set. Deny case (missing fields): standard validation 422. | None yet. |
...
| 4. User submits the challenge form (`2fa_code`) — same `POST /login` route, `throttle:6,1` still applies (shared with step 1, since it's one route). | Browser → server | `validateLogin()`: `_2fa_login` key present → requires only `2fa_code`. Helper merges it with the flashed credentials and re-attempts. Deny case: wrong code → validation error re-rendered on the challenge view, flashed credentials retained for another attempt, subject to **two** independent throttles: the shared `throttle:6,1` route limiter (6/minute, by IP, across both the original login and every retry) *and* `LoginController::throttleKey()`'s per-account limiter for this step specifically (`ThrottlesLogins`' default 5/minute, keyed by account only — no IP component — so a distributed, multi-IP attacker can't reset the budget by rotating source IPs; see "Hardening" below and `architecture.md` Trust Boundaries). | On failure, both limiters are hit; the account-only one is what actually stops a distributed attacker. |
| 5. Correct TOTP or recovery code → `Auth::login()` completes for real. | Server | A used recovery code is single-use and marked used (`used_at`), not removed, on success. | Session established; flashed credential blob cleared; both throttle counters implicitly stop mattering (a fresh login starts a fresh challenge). |

### Hardening: per-account 2FA-code throttle (fixed after ship-check audit)

The 2FA-code step originally relied only on the shared `throttle:6,1` route limiter. Two problems: (1) `TwoFactorLoginHelper::attemptWhen()` throws directly on a wrong code instead of returning `false`, so `AuthenticatesUsers::login()`'s own `incrementLoginAttempts()` call (which normally follows a failed `attemptLogin()`) was never reached — the existing per-account `ThrottlesLogins` machinery was silently bypassed for every wrong-code guess; and (2) even if reached, its default `throttleKey()` is `"email|ip"`, so a distributed attacker rotating source IPs would get a fresh 5-guess budget per IP against a 6-digit (1,000,000-value) TOTP space. `LoginController` now overrides `attemptLogin()` to increment the limiter itself on the helper's exception path, and overrides `throttleKey()` to drop the IP component for this step only (keyed by account alone), while the credential (password) step keeps the inherited `"email|ip"` behavior unchanged.
