...
- **`Money::formatTo()` (locale-aware, `ext-intl`-dependent) is never called anywhere in `app/`** — confirmed by grep (`resources/js/shared/ui/form/MathInput.vue:79`'s `formatToParts` is `Intl.NumberFormat`'s own method, unrelated). Display formatting stays centralized in `resources/js/shared/lib/i18n/format.js` (`toFormattedCurrency()`, `getDecimalPrecision()`), which was extended with a `getDecimalPrecision()` helper for input-side clamping but not otherwise changed — confirming both `specification.md`'s Non-Goal ("no change to format.js's display formatting") and the root `CLAUDE.md`'s rule against introducing an undocumented `ext-intl` dependency.
...
- **Input-side precision clamping is still `RoundingMode::HalfUp`-tolerant at the cast layer, not enforced earlier in the UI for every field.** `MathInput.vue` gained a `precision` prop (Phase 3, FR-6) that clamps user-typed values for most amount fields, but the price field's clamp is a display-precision choice, decoupled from the DB's full scale-10 storage capacity (this is intentional, per the follow-up doc — not itself a bug).
...
- `.ai/docs/specifications/precision-improvements/specification.md` — the FR-1 through FR-7 implementation-handoff spec, all phases marked completed.
