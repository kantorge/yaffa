...
|---|---|---|---|---|---|---|
| List own budgets | `GET /api/v1/budgets` → `index` | `read` | `viewAny` → **always `true`** | — | Query explicitly `where('user_id', ...)` via `$user->budgets()` (`BudgetApiController.php:44`); `#[Authorize('viewAny', Budget::class)]` attribute on `index()` (`BudgetApiController.php:33`) | `budgets.user_id` FK |
| View one budget | `GET /api/v1/budgets/{budget}` → `getItem` | `read` | `view` → `isOwnItem()` (real check) | — | `#[Authorize('view', 'budget')]` attribute on `getItem()` (`BudgetApiController.php:52`) | Route-model-bound by id only — **no** `user_id` scoping at the query level, so a cross-user id resolves the model and is rejected by the Policy (403), not by a 404 |
| Create | `POST /api/v1/budgets` → `store` | `write` | `create` → **always `true`** | `BudgetRequest` — real ownership check on `category_id`/`account_id` (`Rule::exists(...)->where('user_id', ...)`) | `#[Authorize('create', Budget::class)]` attribute on `store()` (`BudgetApiController.php:72`, no-op) | `user_id` auto-set from `$user->budgets()->create()`, not client-supplied (`user_id` not in `$fillable`) |
| Update | `PATCH /api/v1/budgets/{budget}` → `update` | `write` | `update` → `isOwnItem()` (real check) | `BudgetRequest` — same ownership check, re-validated on every update | `#[Authorize('update', 'budget')]` attribute on `update()` (`BudgetApiController.php:87`) | Same route-binding-then-403 shape as `view` |
| Delete | `DELETE /api/v1/budgets/{budget}` → `destroy` | `write` | `delete` → `isOwnItem()` (real check) | — | `#[Authorize('delete', 'budget')]` attribute on `destroy()` (`BudgetApiController.php:98`) | Same shape |

*(Note: as of the Laravel 13 upgrade, controller authorization in this codebase is declared via PHP 8 method/class attributes — `Illuminate\Routing\Attributes\Controllers\Authorize` and `...\Middleware` — rather than in-body `Gate::authorize()` calls or a static `middleware()` method. Functionally identical timing/effect; only the syntax and line-level citations above changed from earlier drafts of this doc.)*
