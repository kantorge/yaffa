...
|---|---|---|---|
| 1. User clicks "New Budget," fills category, optional account, transaction type, amount, and a recurrence rule (frequency/interval/start/end/count/inflation, plus at most one of the mutually-exclusive month-scoped patterns `by_day`+`by_month` / `days_before_month_end` / `last_business_day_of_month`). | Browser (client-side) | None yet. | None. |
...
| 5. `BudgetService::store()` — `$user->budgets()->create($data)`. | Server | `user_id` is set from the `$user->budgets()` relation, not from request data (`user_id` is not in `Budget::$fillable`). | Row inserted; `HasRecurrenceRule::fill()` composes the discrete recurrence fields into the single `rrule` column first. `Budget::booted()`'s `creating` hook computes `active` synchronously (`app/Models/Budget.php:105-107`) via `RecurrenceRuleService::hasOccurrenceOnOrAfter()`. |
...
## 9. One-Time Data Migration: Schedule Recurrence Columns → Single `rrule` Column

- **Actor:** the deploying operator, via `php artisan migrate` (not a user-facing flow). Runs before flow 8 (`2026_08_04_*` sorts before `2026_08_05_*`).
- **Precondition:** a database backup exists (`UPGRADE.md`) — the column drop is irreversible.
- **Success outcome:** every `transaction_schedules` row has a populated `rrule`; `frequency`/`interval`/`count`/`end_date` (and `by_day`/`by_month` where present) are dropped; `budgets` is created with `rrule` directly.

| Step | Boundary crossed | Authz check | Side effect |
|---|---|---|---|
| 1. `2026_08_04_000001_add_rrule_to_transaction_schedules_table` adds nullable `rrule`. | Server (migration runner) | N/A — system migration across all users' data. | Additive DDL only. |
| 2. `2026_08_04_000002_backfill_rrule_on_transaction_schedules_table` rebuilds each row's RRULE from its legacy columns via the query builder (own private assembler, not the Eloquent model). | Server | N/A. Integrity note: UNTIL and COUNT cannot both be present — 3.x request validation already made `end_date`/`count` mutually exclusive (`prohibits`), so the assembler never sees both. | `UPDATE transaction_schedules SET rrule`. `down()` only nulls `rrule`. |
| 3. `2026_08_04_000003_drop_recurrence_columns_from_transaction_schedules_table` counts rows with null/empty `rrule` **before any DDL** and throws `RuntimeException` if any exist. | Server | Deny case: guard throws, no DDL has run (MySQL DDL auto-commits, so ordering is load-bearing). Pinned by `TransactionScheduleRruleMigrationTest::test_drop_migration_refuses_to_run_if_a_row_has_no_rrule`. | Drops the old columns, tightens `rrule` to `NOT NULL`. `down()` restores empty columns only — recurrence is lost on rollback. |
| 4. `2026_08_05_000002` (flow 8) then builds `Budget` rows through the Eloquent model, reading the schedule's virtual attributes (decomposed from the now-populated `rrule`). | Server | N/A. | See flow 8. Couples that migration to the live `HasRecurrenceRule` behavior. |
