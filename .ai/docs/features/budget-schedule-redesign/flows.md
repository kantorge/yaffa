...
| 2. `POST /api/v1/budgets` `{ category_id, account_id?, transaction_type, amount, ... }` | Browser → server | `auth:sanctum` + `verified` + `abilities:write` (`BudgetApiController::middleware()`, `app/Http/Controllers/API/BudgetApiController.php:23-35`). Deny case: a bearer token without `write` gets 403 before the controller runs. | None yet. |
| 3. `Gate::authorize('create', Budget::class)` | Server | `BudgetPolicy::create()` (`app/Policies/BudgetPolicy.php:40-43`) — returns `true` unconditionally for any authenticated user; this is **not** where ownership is enforced. | None. |
