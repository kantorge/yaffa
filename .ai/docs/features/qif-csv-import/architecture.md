...
- **`/imports/parse` and `/imports/file-profiles/suggest` have no dedicated rate limit — only the global API limiter.** These are the two most expensive endpoints in the app (file parsing up to `import_max_rows`, or a paid outbound AI call). They sit in `routes/api.php`, so they inherit the `throttle:api` middleware appended to the `api` group in `bootstrap/app.php` (`$middleware->api(append: [..., 'throttle:api'])`). That limiter is defined in `AppServiceProvider::boot()` as a hard-coded `Limit::perMinute(120)` (6000 in the `testing` environment), keyed by authenticated user ID (IP if unauthenticated), and is not configurable via `.env`/`config()`. See `api-access-and-2fa/architecture.md` for the full description. 120/min is a generous ceiling for these two endpoints, so an authenticated user (or compromised account) can still drive substantial parsing CPU or paid AI spend within the limit; a tighter per-endpoint throttle has not been added. (`routes/api.php:144,150`, `bootstrap/app.php:30-33`, `AppServiceProvider.php:118-120`)
