...
Derived from `architecture.md`, `flows.md`, `permissions.md`, `variables.md` (reverse-engineered from `feat/qif-csv-import`) plus the surviving finding from the prior security audit (no dedicated, tighter rate limit on `/imports/parse` / `/imports/file-profiles/suggest` beyond the global 120/min API limiter). Only load-bearing, deterministic rules are scored — the ones where getting it wrong crosses an authz, data, money, or privacy boundary. Cosmetic/UI-only behavior (tooltips, disabled buttons, wizard pre-fill) is out of scope.
...
| 28 | Rate limiting on `/imports/parse` and `/imports/file-profiles/suggest` | Both routes inherit the global `throttle:api` limiter (`bootstrap/app.php` `api` group → `RateLimiter::for('api')` in `AppServiceProvider::boot()`: hard-coded 120/min, keyed by user ID, IP if unauthenticated); no endpoint-specific tighter limit | Global limit: an authenticated user exceeding 120 requests/min gets 429. Gap: within that ceiling, a user can still drive up to 120 file parses or paid AI calls per minute | `architecture.md` § Known Risks; `variables.md` § Pre-Go-Live Checklist; `routes/api.php:144,150`; `bootstrap/app.php:30-33`; `AppServiceProvider.php:118-120` | unit | partial (`RateLimiterTest.php` pins per-user/IP keying of the `api` limiter only; no test asserts these two routes are throttled or return 429, and the `testing` environment raises the limit to 6000) |
...
1. **No dedicated rate limit on `/imports/parse` or `/imports/file-profiles/suggest`** (row #28). The one surviving finding from the prior security audit, since narrowed: both routes now inherit the global `throttle:api` limiter (hard-coded 120 requests/min per user, `AppServiceProvider::boot()`, applied via the `api` group in `bootstrap/app.php`), so abuse is bounded rather than unlimited. But both endpoints are expensive by design — one runs a parser up to `IMPORT_MAX_ROWS`, the other makes a paid outbound AI call using the user's own key — and 120/min is a generous ceiling for them. A compromised or malicious authenticated account can still drive significant parsing CPU or AI spend within that limit. A tighter per-endpoint throttle would be a fix-then-test item; meanwhile no test asserts these two routes are actually throttled (only the limiter's keying is pinned in `RateLimiterTest.php`). Ranked first because it's the only finding the security audit left open, and it's a live cost/abuse vector, not a theoretical one.
...
8. **No test enforces the file-size cap** (row #7) despite `IMPORT_MAX_FILE_SIZE_MB` being a real, working config value on both upload endpoints. Low severity on its own (Laravel's `File::max()` rule is a well-trodden path), but combined with gap #1 (only the global 120/min limiter, no dedicated one), an untested cap is one refactor away from silently becoming a no-op.
