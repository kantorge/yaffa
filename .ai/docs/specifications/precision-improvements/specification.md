...
Round `MathInput.vue`'s emitted value (`resources/js/shared/ui/form/MathInput.vue:40,56`) using `decimal.js`, so a math-expression evaluation like `"0.1+0.2"` never emits a float-drift artifact (`0.30000000000000004`) — see FR-8 for the rounding ceiling and its warning-toast behavior. Extend `decimal.js` usage to `TransactionItemContainer.vue`'s allocation path (replacing the `toFixed(4)`/remainder-reconciliation workaround at lines 404-436), `TransactionFormStandard.vue`'s `allocatedAmount`/`remainingAmount*` computed properties (lines 802-827), and the API-response parsing layer, which must start parsing FR-4's decimal-string JSON output instead of assuming a JSON number.
...
### FR-8: Decouple input clamping/validation from currency display precision — Phase 5 — ✅ Completed

**Input clamping.** `MathInput.vue` has no per-field `precision` prop. A fixed internal constant, `FLOAT_NOISE_CLEANUP_SCALE = 10`, normalizes floating-point representation noise from `mathjs`'s native-float `evaluate()` (e.g. `0.1 + 0.2` evaluates to `0.30000000000000004`) — it does not enforce any field's real storage capacity. `updateAmount()` wraps the evaluated result in `Decimal` both before and after rounding to `FLOAT_NOISE_CLEANUP_SCALE` and compares the two with `Decimal.equals()`: when they differ, it shows a warning toast (`showWarningToast()`, "The entered value had more decimal places than supported and was rounded."). A value within 10 decimal places — which covers every realistic manually-typed amount — passes through unchanged. `TransactionItem.vue`'s `loadRemainder()` (assigning the item's amount from the remaining unallocated total) similarly applies no decimal-place clamp: it reads the live `amountData` and sums it with `remainingAmount` via `Decimal.plus()`, which introduces no rounding error of its own, so the copied value is exact.

**Display formatting.** `resources/js/shared/lib/i18n/format.js`'s `toFormattedCurrency()` sets `minimumFractionDigits` from the currency's configured `generic_decimal_precision`/`detailed_decimal_precision` (the floor) and `maximumFractionDigits` from a per-field storage-scale constant, `resources/js/shared/lib/money/scale.js`'s `STORAGE_SCALE` (the ceiling — 4 for standard/item amounts and investment fees, 10 for price/rate, mirroring each field's `MoneyCast`/`DecimalCast` declaration in `app/Models/*.php`). `Intl.NumberFormat` trims trailing zeros between the floor and ceiling automatically, so a whole-number amount in a 2-decimal currency still shows `"10.00"` (the floor), a value with real fractional content up to the storage scale shows all of it (e.g. `"12.3456"`), and nothing shows a meaningless `"123.0000"` just because the column can hold four digits. This requires no per-call-site change: every one of the ~40 `toFormattedCurrency()`/`toFormattedCurrency()`-via-DataTables-wrapper call sites already passes `generic` or `detailed` correctly (the `detailed` calls are already, without exception, the rate/price fields).

**Validation.** `TransactionRequest` carries an explicit `decimal:0,<scale>` Laravel rule (matching each field's storage scale) on `amount_from`/`amount_to`/item `amount`/investment `price`/`commission`/`tax`/`dividend`, alongside the existing `max:` magnitude bound; `InvestmentPriceRequest`'s `price` rule carries the equivalent `decimal:0,10`. This is the sole enforcement point for a field's real storage ceiling: a value with more fractional digits than a column supports surfaces as a `422` with a clear field-level error. `MoneyCast`/`DecimalCast::set()` itself still rounds with `RoundingMode::HalfUp` rather than throwing, as a backstop for any write path that bypasses these Form Requests (e.g. a queued job constructing a model directly) — not a gap this FR closes at the cast level.
...
- `app/Http/Requests/TransactionRequest.php` — `decimal:0,<scale>` rules for `amount_from`/`amount_to`/item `amount`/investment `price`/`commission`/`tax`/`dividend`, matching each field's `MoneyCast`/`DecimalCast` scale (FR-8).
- `app/Http/Requests/InvestmentPriceRequest.php` — equivalent `decimal:0,10` rule on `price` (FR-8).
...
- New shared constant module `resources/js/shared/lib/money/scale.js` declaring each field family's storage scale, cross-referenced with the matching `MoneyCast`/`DecimalCast` declaration in `app/Models/*.php` (FR-8).
...
- `resources/js/shared/ui/form/MathInput.vue` — no per-field `precision` prop; a fixed internal `FLOAT_NOISE_CLEANUP_SCALE` constant and a `showWarningToast()` call on actual truncation (FR-8).
- `resources/js/transactions/components/form/TransactionItem.vue`'s `loadRemainder()` — reads the live `amountData` and sums via `Decimal.plus()` with no decimal-place clamp (FR-8).
...
- **Backend, shipped:** `TransactionApiControllerTest` gained `test_store_standard_accepts_amount_at_exact_decimal_4_scale`, `test_store_standard_rejects_amount_with_excess_decimal_places`, `test_store_investment_rejects_price_with_excess_decimal_places`, `test_store_investment_rejects_commission_with_excess_decimal_places`, `test_store_investment_rejects_dividend_with_excess_decimal_places` — closes `tests.md` gap #3 by asserting the `decimal:0,<scale>` rules reject over-precise input at the validation layer (a `422`, not a silently-rounded `200`) while a value at exactly the field's scale still passes. `InvestmentPriceApiV1Test` gained the equivalent pair (`test_v1_price_rejects_excess_decimal_places`, `test_v1_price_accepts_value_at_exact_decimal_scale`) for `InvestmentPriceRequest`.
- **Frontend, verified manually:** confirmed by inspection and a production build (`vendor/bin/sail npm run build`) that `toFormattedCurrency()`'s min/max computation trims correctly (a whole-number `generic`-precision-2 amount still shows two decimals; a `detailed`-precision price with real fractional content up to 10 places shows all of it); that `MathInput.vue` never alters a manually-typed or `loadRemainder()`-computed value with up to 10 decimal places (e.g. `1.1111111111` round-trips unchanged); that a value genuinely exceeding 10 decimals (or carrying `evaluate()`-induced float noise, e.g. `0.1 + 0.2`) is rounded to 10 places and triggers `showWarningToast()`; and that `loadRemainder()` copies the exact live `amountData` + `remainingAmount` sum with no truncation.
- **Not shipped — no test framework exists to write it in.** Automated JS coverage for `format.js`'s floor/ceiling split, `MathInput`'s truncation-warning behavior, and `loadRemainder()`'s exact-sum behavior was planned, but this repository has no JS test runner configured (no Jest/Vitest, no existing `*.test.js`, no `test` script in `package.json`) — adding one is a new-dependency decision `CLAUDE.md` reserves for explicit user approval, not something to introduce as a side effect of one feature's coverage. All three are documented here as the acceptance bar a future JS test suite should pin.
...
7. `TransactionRequest`/`InvestmentPriceRequest` validation is the sole enforcement point for a field's storage scale (not the currency's configured display precision, and not backstopped by a matching frontend clamp), and `toFormattedCurrency()` never forces a fixed digit count — it always trims between the currency's precision (floor) and the field's storage scale (ceiling) (FR-8).
8. `MathInput.vue` has no per-field `precision` prop and never silently truncates a manually-typed or programmatically-computed value within 10 decimal places; when it does round (float noise, or a value genuinely exceeding 10 decimals), it always shows a warning toast rather than truncating silently (FR-8).
...
### Phase 5 — Decouple input clamping/validation from currency display precision — ✅ Completed

**Scope:** FR-8.
**Depends on:** Phase 3 (the `MathInput`/`format.js` surfaces this phase touches must already exist).
**Dependency approval:** none — no new library; `decimal.js` and `Intl.NumberFormat` are already in place.
**State at end of phase:** a currency's `generic_decimal_precision`/`detailed_decimal_precision` governs display only (a floor, paired with each field's storage scale as ceiling); `TransactionRequest`/`InvestmentPriceRequest` validate against each field's real storage scale, the sole enforcement point for it; `MathInput.vue` never silently changes a value within 10 decimal places; no display ever pads a value with digits it doesn't have.
**Shipped:** new `resources/js/shared/lib/money/scale.js` (`STORAGE_SCALE.AMOUNT = 4`, `STORAGE_SCALE.PRICE = 10`), cross-referenced in a comment against the `MoneyCast`/`DecimalCast` declarations it mirrors, and used by `format.js`'s display ceiling. `format.js`'s `toFormattedCurrency()` sets `minimumFractionDigits` from the currency's configured precision (floor) and `maximumFractionDigits` from `STORAGE_SCALE` (ceiling) instead of forcing them equal; the dead `min_digits`/`max_digits` currencySettings fallback was removed with it. `TransactionRequest` gained a `decimal:0,4` or `decimal:0,10` rule (matching each field's scale) on `items.*.amount`, `config.amount_from`/`amount_to` (all three transaction-type branches), `config.commission`/`tax`/`price`/`dividend`; `InvestmentPriceRequest`'s `price` rule gained the equivalent `decimal:0,10`. `TransactionRequest` also applies `decimal:0,4` to `config.quantity` for BUY/SELL and ADD_SHARES/REMOVE_SHARES, matching `transaction_details_investment.quantity`'s `DecimalCast` scale. `MathInput.vue` has no per-field `precision` prop; `updateAmount()` rounds to a fixed internal `FLOAT_NOISE_CLEANUP_SCALE = 10` purely to normalize floating-point noise from `mathjs`'s `evaluate()`, and calls `showWarningToast()` when `Decimal.equals()` shows the rounding actually changed the value. `TransactionItem.vue`'s `loadRemainder()` reads the live `amountData` and sums via `Decimal.plus()` with no decimal-place clamp, since exact decimal addition introduces no error to clean up. All backend tests, Pint, and PHPStan pass, and the frontend build is clean; see the Testing Requirements section above for the planned JS-level coverage not shipped (no JS test framework exists in this repository to write it in, flagged for a separate decision rather than silently adding one).
