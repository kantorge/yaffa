...
- **Shipped:** `TransactionApiControllerTest` gained `test_store_standard_accepts_amount_at_exact_decimal_4_scale`, `test_store_standard_rejects_amount_with_excess_decimal_places`, `test_store_investment_rejects_price_with_excess_decimal_places`, `test_store_investment_rejects_commission_with_excess_decimal_places`, `test_store_investment_rejects_dividend_with_excess_decimal_places` — closes `tests.md` gap #3 by asserting the new `decimal:0,<scale>` rules reject over-precise input at the validation layer (a `422`, not a silently-rounded `200`) while a value at exactly the field's scale still passes. `InvestmentPriceApiV1Test` gained the equivalent pair (`test_v1_price_rejects_excess_decimal_places`, `test_v1_price_accepts_value_at_exact_decimal_scale`) for `InvestmentPriceRequest`.
- **Not shipped — no test framework exists to write it in.** A `format.js` unit test for the floor/ceiling `minimumFractionDigits`/`maximumFractionDigits` split was planned, but this repository has no JS test runner configured (no Jest/Vitest, no existing `*.test.js`, no `test` script in `package.json`) — adding one is a new-dependency decision `CLAUDE.md` reserves for explicit user approval, not something to introduce as a side effect of one function's coverage. The floor/ceiling logic was verified manually instead (see below) and via the frontend production build (`vendor/bin/sail npm run build`) completing without error.
- Manual verification performed: confirmed `toFormattedCurrency()`'s new min/max computation trims correctly by inspection of the rewritten function against representative cases (a whole-number `generic`-precision-2 amount still shows two decimals; a `detailed`-precision price with real fractional content up to 10 places shows all of it; nothing pads beyond a currency's configured precision when the underlying value doesn't carry it). A full manual browser walkthrough (entering a high-precision value into a live `MathInput` field) was not performed as part of this pass — flagged as outstanding manual QA before release.
...
**Shipped:** new `resources/js/shared/lib/money/scale.js` (`STORAGE_SCALE.AMOUNT = 4`, `STORAGE_SCALE.PRICE = 10`), cross-referenced in a comment against the `MoneyCast`/`DecimalCast` declarations it mirrors. `TransactionFormStandard.vue`'s `amountFromPrecision`/`amountToPrecision` and `TransactionFormInvestment.vue`'s `pricePrecision`/`moneyPrecision` now return the matching `STORAGE_SCALE` constant instead of calling the now-removed `getDecimalPrecision()`; `TransactionFormStandard.vue`'s now-unused `amountFromCurrency` computed property (its only caller) was removed alongside it. `format.js`'s `toFormattedCurrency()` sets `minimumFractionDigits` from the currency's configured precision (floor) and `maximumFractionDigits` from `STORAGE_SCALE` (ceiling) instead of forcing them equal; the dead `min_digits`/`max_digits` currencySettings fallback was removed with it. `TransactionRequest` gained a `decimal:0,4` or `decimal:0,10` rule (matching each field's scale) on `items.*.amount`, `config.amount_from`/`amount_to` (all three transaction-type branches), `config.commission`/`tax`/`price`/`dividend`; `InvestmentPriceRequest`'s `price` rule gained the equivalent `decimal:0,10`. `quantity` was left unclamped and unvalidated for decimal places throughout, per FR-6's existing carve-out (not a currency amount). All Phase 5 backend tests, Pint, and PHPStan pass; see the Testing Requirements section above for the one planned test not shipped (`format.js` unit coverage — no JS test framework exists in this repository to write it in, flagged for a separate decision rather than silently adding one).
