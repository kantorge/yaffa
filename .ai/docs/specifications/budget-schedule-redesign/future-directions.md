...
This is the same principle [background.md](background.md) applies when explaining why `Budget` doesn't duplicate a schedule's categorized amounts into a second record (don't duplicate real category/amount facts — express deltas instead), applied one level up. It has two direct consequences:
