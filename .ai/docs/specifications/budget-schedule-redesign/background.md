...
- **The critical realization**: a scheduled standard withdrawal/deposit with categorized items already *is* valid budget input — the categorized amounts live on the same `TransactionItem` rows the schedule uses. A telco bill scheduled transaction split across TV/broadband categories is a concrete example: those category amounts are already real budget data, sitting right there on the schedule's items. There is no independent "budget" decision to make for it; requiring a second flag — or duplicating the categorized amounts into a separate `Budget` record — just creates a place for the same fact to drift out of sync when the amount changes.
...
`Budget.account_id` is nullable (FR-4), which raises a real design question: how does an account-agnostic budget relate to a per-account one for the *same category*, given YAFFA already supports both, and `account_monthly_summaries.account_entity_id` is already nullable for exactly this reason? Two real, coexisting use cases motivate this:
