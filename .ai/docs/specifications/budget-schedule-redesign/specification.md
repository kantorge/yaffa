...
- Removing the `account_monthly_summaries` precomputed-cache concept (`app/Models/AccountMonthlySummary.php`) — FR-9 fixes the *performance* of the forecast calculation that feeds it (and FR-3 repoints its budget bucket at `Budget` instead of `budget=true` transactions), but the table, its daily full-recalculation cron, and its event-driven partial-invalidation wiring (`TransactionService`, `ProcessTransactionUpdated`, `InvestmentService`) all remain exactly as they are today. Only the forecast/schedule-instance calculation was profiled and shown fast enough for on-demand use (see [forecast-performance.md](forecast-performance.md)); whether the fact-data calculation could also be computed live, at request time, for the current-balance and cashflow-report paths, was never measured — so whether the cache could eventually be retired is an open question, not a decision this redesign makes either way. See [future-directions.md](future-directions.md) for the note on revisiting this later.
