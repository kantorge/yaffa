...
**Revised understanding vs. the original hypothesis above** *(superseded — see "Correction" below)*: both original suspects are confirmed, but the specific mechanism for #1 is now precise — it isn't just "replicate() is called a lot," it's "replicate() silently drops loaded relations, causing every virtual instance to independently re-trigger relation queries it should never have needed to make at all."

**Implication for the fix** *(the relation-drop framing here is superseded — see "Correction" below; the price-batching and N+1 diagnosis stand)*: caching the per-investment latest price *once per job run* (addressing the `Investment::find()`/`getLatestPrice()` N+1) and either re-attaching loaded relations after `replicate()` or restructuring virtual instances as plain arrays/DTOs that carry the already-resolved `config` data directly (avoiding the relation-drop problem entirely) would eliminate the overwhelming majority of the ~1,200+ redundant queries seen in a single job — and, since query time was only ~20-25% of the total, likely a proportionally larger share of wall-clock time once the cascading PHP-side rehydration those queries trigger is also removed.

## Status — ✅ Fixed (FR-9 / Phase 2 complete)

Adopted into [specification.md](specification.md) as **FR-9** and implemented as Phase 2 of the rollout plan (Section 12), ahead of FR-8 and the rest of the 4.0.0 work. `Transaction::scheduleInstances()` no longer calls `replicate()` per occurrence (see `App\Support\ScheduleInstance`), and `CalculateAccountMonthlySummary::getInvestmentValueForecastData()` no longer calls `Investment::find()`/`getLatestPrice()` per investment per forecast month. Verified by regression tests asserting query count independent of occurrence count/forecast horizon (`tests/Unit/Models/TransactionScheduleInstancesTest.php`, `tests/Unit/Jobs/CalculateAccountMonthlySummaryTest.php`), not just wall-clock improvement. FR-8's inflation-compounding logic (Phase 4) is now clear to build on top of this fixed implementation.

## Correction (post-implementation, 2026-07-25)

The claim above that "`Transaction::replicate()` does not carry over already-loaded Eloquent relations" is **incorrect** for the framework version actually installed in this project (`laravel/framework` v12.64.0). `Model::replicate()` (`vendor/laravel/framework/src/Illuminate/Database/Eloquent/Model.php:1898-1904`) explicitly calls `$instance->setRelations($this->relations)`, so a replicated model's already-loaded relations (including a `MorphTo` like `config`) *are* preserved, and accessing them triggers no additional query. A direct reproduction against this exact framework version, using a `MorphTo` relation structurally identical to `Transaction::config()`, confirmed zero queries across a replicate-then-access loop.

This does not change the outcome: the `Investment::find()`/`getLatestPrice()` per-month N+1 in `getInvestmentValueForecastData()` (point 2 above) is a real, independently-confirmed bug regardless of relation behavior, and the ~75-80% PHP-side overhead the profiling measured is consistent with `replicate()`'s own per-occurrence cost (attribute-array copy, cast re-application, model event dispatch) rather than relation re-querying specifically. FR-9 was implemented as specified — a non-Eloquent DTO (`App\Support\ScheduleInstance`) replacing `replicate()` in `Transaction::scheduleInstances()`, plus batched price lookups in `getInvestmentValueForecastData()` — and both fixes stand on their own merits. Only the specific "`config` re-queried ~600 times because replicate() drops relations" mechanism described above should be treated as unconfirmed/likely incorrect for this codebase's actual Laravel version; what exactly produced the `transaction_details_investment` query repeating in the original profiling run was not re-diagnosed, since it doesn't change which fix was needed.
