...
(One theory floated during investigation — that `Transaction::replicate()` silently drops already-loaded Eloquent relations, forcing each virtual instance to independently re-query its own `config` relation — turned out not to apply to the Laravel version actually installed in this project, `laravel/framework` v12.64.0: `Model::replicate()` does preserve loaded relations via `setRelations()`. The PHP-side overhead measured above comes from `replicate()`'s own per-call cost — attribute-array copying, cast re-application, event dispatch — not from relation re-querying. This doesn't change the fix below, which replaces `replicate()` entirely regardless of which specific cost dominates.)
