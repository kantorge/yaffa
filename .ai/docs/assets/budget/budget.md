...
Like a schedule, the recurrence-shape fields (frequency, interval, end date, count, and the
month-scoped pattern) are stored as a single RFC 5545 RRULE string in one `rrule` column, not one
column per field; `start_date` and `inflation` remain their own columns — see
